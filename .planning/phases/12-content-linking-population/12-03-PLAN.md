...
Document the IDs in a comment at the top of the test file.

Note: If certain scenarios don't have real data, the test should skip with `test.skip()` and a clear message.
  </action>
  <verify>
Document the detail IDs found (or note which scenarios need seeding)
  </verify>
  <done>
- Four detail IDs identified (or documented as needing seed data)
...
test.describe('Content Linking Scenarios', () => {
  ...
  test.describe('Linked Detail (MRM with borrowed RANZ content)', () => {
    test('shows borrowed 3D model with source attribution', async ({ page }) => {
      await page.goto('/planner/[substrate]/[category]/[linked-mrm-id]');
      await expect(page.getByRole('heading')).toBeVisible({ timeout: 10000 });

      // Should have 3D viewer visible (borrowed from RANZ)
      const threeDTab = page.getByRole('tab', { name: /3d|model/i });
      if (await threeDTab.isVisible()) {
        await threeDTab.click();
      }
      ...
      // Should show source attribution (RANZ provided this model)
