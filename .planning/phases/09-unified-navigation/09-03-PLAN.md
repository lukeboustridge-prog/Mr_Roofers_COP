...
Modify the query to include these counts/flags. Options:
1. Add subqueries for each capability (simple but may be slower)
2. Add JOINs with count (better performance)
3. Add as optional fields and compute client-side (simplest for now)

For initial implementation, use LEFT JOINs to get counts and convert to boolean:
...
Also add source counts for the tabs - return separate counts for each source alongside data:
...
- Checking capability filters filters the list and updates URL
- URL changes persist on page refresh
- Browser back button restores previous filter state
  </verify>
  <done>
Topic page has fully integrated source tabs and capability filters with URL state persistence.
  </done>
</task>
