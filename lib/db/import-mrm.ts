...
import { substrates, categories, details, detailSteps, warningConditions, detailFailureLinks } from './schema';
...
async function importMrmData() {
  ...
  // Clear existing MRM content data (preserve case law — managed by scripts/import-case-law-pdfs.ts)
  console.log('Clearing existing MRM data...');
  ...
  console.log('Existing MRM data cleared (case law preserved)\n');
  ...
  for (const detail of detailsData) {
    ...
    processedDetails.push({
      ...
      images: detail.images.length > 0 ? detail.images.map(img => `${R2_PUBLIC_URL}/images/mrm/${img}`) : null,
  ...
  // NOTE: Case law (failure_cases + detailFailureLinks) is managed separately by:
  //   1. npm run db:import-case-law   (scripts/import-case-law-pdfs.ts — imports all 73 PDFs)
  //   2. npm run db:link-case-law     (scripts/link-case-law-to-details.ts — creates detail links)
  // After running this MRM import, re-run those scripts to restore case law links.
  ...
  console.log('\nNote: Run scripts/import-case-law-pdfs.ts and scripts/link-case-law-to-details.ts to restore case law links.');
}
